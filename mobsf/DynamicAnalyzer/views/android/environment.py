...
class EmulatorFormatter(logging.Formatter):
    def format(self, record):
        if hasattr(record, 'emulator'):
            record.msg = f"[{record.emulator}] {record.msg}"
        return super().format(record)

def setup_emulator_logger():
    logger = logging.getLogger(__name__)
    if not logger.handlers:
        handler = logging.StreamHandler()
        formatter = EmulatorFormatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        handler.setFormatter(formatter)
        logger.addHandler(handler)
        logger.setLevel(logging.INFO)
    return logger

logger = setup_emulator_logger()
...
class Environment:
    ...
    def log(self, level, message):
        ...
        log_func(message, extra={'emulator': self.emulator_name})

    def wait(self, sec):
        """Wait in Seconds."""
        if sec > 0:
            self.log('Waiting for %s seconds...', str(sec))
            time.sleep(sec)

    def check_connect_error(self, output):
        """Check if connect failed."""
        if b'unable to connect' in output or b'failed to connect' in output:
            self.log('%s', output.decode('utf-8').replace('\n', ''))
            return False
        return True
    ...
    def connect(self):
        ...
        if not self.identifier:
            return False
        self.log('Connecting to Android %s', self.identifier)
    ...
    def connect_n_mount(self):
        ...
        if not self.run_subprocess_verify_output([get_adb(),
                                                 'connect',
                                                  self.identifier]):
            return False
        self.log('Restarting ADB Daemon as root')
        if not self.run_subprocess_verify_output([get_adb(),
                                                  '-s',
                                                  self.identifier,
                                                  'root']):
            return False
        self.log('Reconnecting to Android Device')
        ...
        self.log('Performing System check')
    ...
    def install_apk(self, apk_path, package, reinstall):
        ...
        if self.is_package_installed(package, '') and reinstall != '0':
            self.log('Removing existing installation')
        ...
        self.log('Installing APK - %s', package)
        # Install APK
        out = self.adb_command([
            ...
            apk_path], False, True)
        if not out:
            self.log('adb install failed')
            return False, 'adb install failed'
        
        # Change battery optimization settings to "Unrestricted"
        self.log('Changing battery optimization settings to "Unrestricted" for %s', package)
        ...
        if accessibility_permission:
            self.log('Accessibility permission found in installed APK')
            # Run commands to configure accessibility settings
            self.log('Configuring accessibility settings...')
            self.execute_accessibility_commands()
        else:
            self.log('Accessibility permission not found in installed APK')
    ...
    def check_accessibility_permission(self, package):
        ...
        try:
            ...
            if out is None:
                self.log('Error executing adb command to check permissions')
                return False
    ...
    def execute_accessibility_commands(self):
        ...
        self.log('Opening Accessibility settings')
        ...
        self.log('Navigating through Accessibility settings')
    ...
    def configure_proxy(self, project, request):
        ...
        self.log('Starting HTTPS Proxy on %s', proxy_port)
        ...
        if proxy_ip:
            if version < 4.4:
                logger.warning('Please set Android VM proxy as %s:%s',
                               proxy_ip, proxy_port)
                return
            self.log('Setting Global Proxy for Android VM')
            self.adb_command(
                ['settings',
                 'put',
                 'global',
    ...
    def unset_global_proxy(self):
        ...
        self.log('Removing Global Proxy for Android VM')
        self.adb_command(
            ['settings',
             'delete',
             'global',
    ...
    def enable_adb_reverse_tcp(self, version):
        ...
        if not version >= 5:
            return
        proxy_port = settings.PROXY_PORT
        self.log('Enabling ADB Reverse TCP on %s', proxy_port)
        ...
        try:
            ...
            if b'error: closed' in stderr:
                ...
                self.log(stderr.decode('utf-8').replace('\n', ''))
        except Exception:
            logger.exception('Enabling ADB Reverse TCP')

    def start_clipmon(self):
        """Start Clipboard Monitoring."""
        self.log('Starting Clipboard Monitor')
    ...
    def get_environment(self):
        ...
        if b'EMULATOR' in out:
            self.log('Found Android Studio Emulator')
            return 'emulator'
        elif (b'genymotion' in out.lower()
                or any(char.isdigit() for char in ver)):
            self.log('Found Genymotion x86 Android VM')
            return 'genymotion'
        elif b'corellium' in out:
            self.log('Found Corellium ARM Android VM')
            return 'corellium'
        else:
            logger.warning(
    ...
    def system_check(self, runtime):
        ...
        try:
            try:
                ...
                if api:
                    self.log('Android API Level '
                                'identified as %s', api)
                    if int(api) > ANDROID_API_SUPPORTED:
                        self.log('This API Level is not supported'
            ...
            if b'Read-only' in stderr:
                self.log(err_msg)
                if runtime == 'emulator':
                    self.log('Please start the AVD as per '
                                 'MobSF documentation!')
                return False
        except Exception:
            self.log(err_msg)
    ...
    def launch_n_capture(self, package, activity, outfile):
        ...
        self.log('Activity screenshot captured')
    ...
    def is_mobsfyied(self, android_version):
        ...
        self.log('Environment MobSFyed Check')
    ...
    def mobsfy_init(self):
        ...
        self.log('Android Version identified as %s', version)
        try:
            if version < 5:
    ...
    def xposed_setup(self, android_version):
        ...
        clip_dump_apk = os.path.join(self.tools_dir,
                                     xposed_dir,
                                     'ClipDump.apk')
        self.log('Installing MobSF Clipboard Dumper')
        self.adb_command(['install', '-r', clip_dump_apk])
        if android_version < 5:
            self.log('Installing Xposed for Kitkat and below')
            ...
            droidmon = os.path.join(self.tools_dir,
                                    xposed_modules,
                                    'Droidmon.apk')
            self.log('Installing Droidmon API Analyzer')
            self.adb_command(['install', '-r', droidmon])
            self.log('Copying Droidmon hooks config')
            self.adb_command(['push', hooks, '/data/local/tmp/'])
        else:
            self.log('Installing Xposed for Lollipop and above')
            xposed_apk = os.path.join(self.tools_dir,
                                      xposed_dir,
        ...
        bluepill = os.path.join(self.tools_dir,
                                xposed_modules,
                                'AndroidBluePill.apk')
        self.log('Installing JustTrustMe')
        self.adb_command(['install', '-r', justrustme])
        self.log('Installing SSLUnpinning')
        self.adb_command(['install', '-r', sslunpin])
        self.log('Installing ProxyOn')
        self.adb_command(['install', '-r', proxyon])
        self.log('Installing RootCloak')
        self.adb_command(['install', '-r', rootcloak])
        self.log('Installing Android BluePill')
        self.adb_command(['install', '-r', bluepill])
        self.log('Launching Xposed Framework.')
    ...
    def frida_setup(self):
        ...
        self.log('Android OS architecture identified as %s', arch)
        if arch in ['armeabi-v7a', 'armeabi']:
            ...
            self.log('Make sure a Genymotion Android x86 VM'
        ...
        self.log('Copying frida server for %s', frida_arch)
    ...
    def run_frida_server(self):
        ...
        if b'fd_server' in check:
            self.log('Frida Server is already running')
            return
        ...
        self.log('Waiting for 2 seconds...')
        time.sleep(2)
