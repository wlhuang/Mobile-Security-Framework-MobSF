...
def view_report_acube(request, checksum, api=False):
    """Dynamic Analysis Report Generation."""
    logger.info('Dynamic Analysis Report Generation')
    try:
        droidmon = {}
        ...
        # Read the contents of the frida_logs file
        with open(fd_log, 'r') as f:
            frida_logs = f.read()
        ...
        context = {'hash': checksum,
                   ...
                   'base64_strings': b64_strings,
                   'trackers': trackers,
                   'frida_logs': frida_logs,  # Store the actual mobsf_frida_out.txt content here in context dictionary
                   'runtime_dependencies': list(deps),
                   'package': package,
                   'version': settings.MOBSF_VER,
                   'title': 'Dynamic Analysis'}
