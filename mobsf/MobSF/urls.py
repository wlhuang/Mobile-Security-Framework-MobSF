...
urlpatterns = [
    ...
    re_path(r'^api/v1/dynamic/acube_JSON$', api_dz.api_dynamic_report),                 #Url added by Xavier. Basically just copy paste from report_json but change name only.
...
if settings.API_ONLY == '0':
    urlpatterns.extend([
        ...
        re_path(r'^android_dynamic_appsavailable/(?P<checksum>[ ]{0})$',
                dz.dynamic_analyzer_appsavailable,
                name='dynamic_analyzer_appsavailable'),
        re_path(r'^android_dynamic_appsavailable/(?P<checksum>[0-9a-zA-Z_\-$\{\}]{11})/(?P<identifier>[0-9a-zA-Z_\-$\{\}]{13})/$',
                dz.dynamic_analyzer_appsavailable,
                name='dynamic_analyzer_appsavailable'),
